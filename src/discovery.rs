...
const FIXED_PATHS: &[(&str, &str)] = &[
    ...
    ("AGENTS.override.md", "AGENTS.override.md — OpenAI Codex overrides"),
    ...
    ("llms-full.txt", "llms-full.txt — complete LLM documentation"),
];
...
pub fn discover(root: &Path) -> Vec<DiscoveredFile> {
    ...
    if claude_rules.is_dir() {
        if let Ok(entries) = std::fs::read_dir(&claude_rules) {
            for entry in entries.flatten() {
                let p = entry.path();
    ...
    if gh_instructions.is_dir() {
        if let Ok(entries) = std::fs::read_dir(&gh_instructions) {
            for entry in entries.flatten() {
                let p = entry.path();
